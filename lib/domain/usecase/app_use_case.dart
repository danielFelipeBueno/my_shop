...
class AppUseCase {
  ...
  Future<List<Product>> getMainProducts(String userId) async {
    ...
    final mainProducts = jsonDecode(utf8.decode(mainProductsResponse.bodyBytes));
    if (mainProducts['data'].length == 0){
      return [];
    }
    final products = mainProducts['data'].map<Product>(
  ...
  Future<List> getHome()async{
    ...
    final products = mainHome['products'].map<Product>((product)=>Product.fromJson(product)).toList();
    return [ads,products];
  }
}